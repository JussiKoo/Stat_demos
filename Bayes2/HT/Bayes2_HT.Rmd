...
ppc_ecdf_overlay(mobile$ict_min, t_ennuste$t_ennuste[1:25, ])

n_plot <- 25

it <- seq(0, 17, by = 0.05)

ecdf_values <- matrix(0, ncol = 2+n_plot, nrow = length(it))
  
ecdf_values[,1] <- ecdf(mobile$ict_max)(it)
ecdf_values[,2] <- ecdf(mobile$ict_min)(it)

sample_index <- sample(1:nrow(mobile), n_plot, replace = FALSE)

for (i in 1:n_plot) {
  ecdf_values[,2+i] <- ecdf(t_ennuste$t_ennuste[sample_index[i],])(it)
}
