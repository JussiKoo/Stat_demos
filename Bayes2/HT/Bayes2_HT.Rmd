...
Mallin sopivuutta suhteessa aineistoon on haastavaa tarkastella, sillä todellisia puhelinten käyttöikiä ei tunneta. Posterioriennusteita kyselyyn vastaajien puhelimen käyttöiälle voidaan verrata tunnettuihin ala- ja ylärajoihin esimerkiksi kertymäfunktioiden avulla \@ref(fig: .
...
```{r post_pred, echo=FALSE, fig.cap="Kertymäfunktiot käyttöiän minimeille, maksimeille ja 25 posterioriennusteelle"}
...
mean(mobile$ict_min < 2)

mean(apply(t_ennuste$t, 2, min) < 2)
...
ppc_dens_overlay(mobile$ict_mean, t_ennuste$t[1:50,])
