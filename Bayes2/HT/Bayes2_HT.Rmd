...
data{
  ...
  int<lower=1, upper=6> age[n];
  int<lower=1, upper=2> sex[n];
  //int<lower=1, upper=4> region[n];
  int<lower=1, upper=4> brand[n];
...
generated quantities{
  real t_ennuste[n];
  for (i in 1:n) t_ennuste[i] = gamma_rng(alpha, exp(-logtheta[i]));
}"
...
model{
  ...
  beta_age ~ normal(0, 100);
  beta_sex ~ normal(0, 100);
  beta_region ~ normal(0, 100);
  beta_brand ~ normal(0, 100);
  beta_income ~ normal(0, 100);
  
  for (i in 1:n) {
    target += log_diff_exp(exponential_lcdf(t_max[i] | exp(-logtheta[i])),
    exponential_lcdf(t_min[i] | exp(-logtheta[i])));
  }
}
generated quantities{
  real t_ennuste[n];
  for (i in 1:n) t_ennuste[i] = exponential_rng(exp(-logtheta[i]));
}"
...
fit <- stan(
  ...
  data = list(
    ...
    age = as.integer(mobile$age),
    sex = as.integer(mobile$sex),
    #income = as.integer(mobile$income),
    #region = as.integer(mobile$region),
    brand = as.integer(mobile$brand_prev)
  ),
  iter = 4000,
  chains = 2,
  refresh = 0,
...
t_ennuste <- rstan::extract(fit, pars = c("t_ennuste"))
```
# Tulokset
...
for (i in 1:n_plot) {
  ecdf_values[,i] <- ecdf(t_ennuste$t_ennuste[sample_index[i],])(it)
}
...
```{r}
mean(apply(t_ennuste$t_ennuste, 2, min) < mobile$ict_min)

mean(apply(t_ennuste$t_ennuste, 2, max) > mobile$ict_max)

ppc_dens_overlay(mobile$ict_mean, t_ennuste$t_ennuste)

plot(fit, plotfun="trace", pars=c("beta_age"))

plot(fit, plotfun="trace", pars=c("t_ennuste[1]"))
```
