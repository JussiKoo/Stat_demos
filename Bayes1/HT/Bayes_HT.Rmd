...
Taulukosta \@ref(tab:Malli1) voidaan huomata se, että *ITPA*-muuttujan 95%-posterioriväli sisältää nollan joten kyseisellä muuttujalla ei tälläkään perusteella ole suurta vaikutusta tuloihin. Taulukoista \@ref(tab:Malli2) ja \@ref(tab:Malli3) voidaan huomata, että prioreilla oli jonkin verran vaikutusta regressiokertoimien posteriorikeskiarvoihin. Erityisesti priorien asettaminen pienentää 95%-posteriorivälejä kaikille kertoimille. Joka tapauksessa malleissa 2 ja 3 kaikki regressiokertoimet ovat selvästi positiivisia. Taulukosta \@ref(tab:LOO) käy ilmi, että malli 2 on paras PSIS-LOO-menetelmän tuottamien ELPD-arvojen perusteella. Joskin mallien välillä ei ole suurta eroa sillä ELPD-arvot ovat likimain samat ja mallien 1 ja 3 ELPD-arvojen keskivirheet ovat melko suuret. Taulukosta \@ref(tab:LOO) käy ilmi, että malli 2 on paras PSIS-LOO-menetelmän tuottamien ELPD-arvojen perusteella. Joskin mallien 1 ja 2 välillä ei ole suurta eroa mallin 1 ELPD-arvon ja sen keskivirheen takia. Mallin 2 voisi sanoa olevan parempi kuin mallin 3 joka oli siis sama malli oletusprioreilla.
...
```{r Posterioriennuste, fig.cap="Mallin 2 posterioriennustejakaumia vs. havaintojen jakauma", message=FALSE, fig.width=5, fig.height=2}
pp_check(fit2) + xlim(-20000, 100000)
```

# Tulkinnat

Tulkitaan vielä seuraavaksi mallia 2 tarkemmin.

# Pohdintaa

Jos mallien sovittamista tähän aineistoon jatkettaisiin, olisi syytä tutkia jotakin mallia joka ottaa huomioon sen, että tulot ovat epänegatiivisia. Yksi vaihtoehto olisi olettaa, että havainnot noudattavat alhaalta katkaistua normaalijakaumaa.
