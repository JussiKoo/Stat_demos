---
title: "Bayes-tilastotiede 1 harjoitustyö"
author: "Jussi Kauppinen"
date: "`r Sys.Date()`"
output: bookdown::pdf_document2
bibliography: references.bib
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = FALSE
)
library(ggplot2)
library(brms)
library(bookdown)
```

```{r, include=FALSE}
siemenluku <- 8547 # LAITA TÄHÄN OPISKELIJANUMEROSI
# jos set.seed(siemenluku antaa virheilmoituksen), 
# vaihda siemenluvuksi opiskelijanumerosi neljä viimeistä numeroa
set.seed(siemenluku)
library("rlang")
library("haven")
library("tidyr")
library("dplyr")

d <- read_spss("daF2076_eng.por") |> 
  select(
    arvosana = K82,
    sukupuoli = T3,
    koulutustaso = BV2_1,
    tulot = BV4_1,
    ITPA = L17,
  ) |> 
  mutate(
    sukupuoli = factor(sukupuoli, labels = c("Mies", "Nainen")),
    koulutustaso = recode_factor(
      factor(koulutustaso),
      "2" = "Korkeintaan toisen asteen koulutus",
      "3" = "Korkeintaan toisen asteen koulutus",
      "4" = "Korkeintaan toisen asteen koulutus",
      "5" = "Alempi korkeakouluaste tai alin korkea-aste",
      "6" = "Alempi korkeakouluaste tai alin korkea-aste",
      "7" = "Ylempi korkeakouluaste",
      "8" = "Ylempi korkeakouluaste"
    )
  ) |> 
  slice(sample(seq_len(n()), size = 300)) |> 
  # käytetään vain täysiä havaintorivejä yksinkertaisuuden vuoksi
  drop_na()
```

# Datan analysointia ja visualisointia

Aineistona tässä harjoitustyössä käytetään Elämänkulku 1971–2002 -aineistoa [@elama].

```{r kuva1, fig.cap="Tulojen jakauma ja gamma-jakauma", fig.height=2.5, fig.width=4}
ggplot(data=d, mapping=aes(tulot))+
  geom_density()+
  stat_function(fun = dgamma, args = list(shape = 4.5, rate = 4.5/24000), color = "blue", linewidth = 1)
```

Kuvasta \@ref(fig:kuva1) käy ilmi, että tulojen voitaisiin ajatella noudattavan gamma-jakaumaa.

```{r kuva2, fig.cap="Koulukeskiarvon yhteys tuloihin", fig.height=2.5, fig.width=4}
ggplot(data=d, mapping=aes(x=arvosana, y=tulot, colour = sukupuoli))+
  geom_point()
```

Sukupuolella näyttäisi olevan vaikutusta tuloihin kuvan \@ref(fig:kuva2) perusteella. Peruskoulun päättötodistuksen keskiarvolla näyttää olevan melko samanlainen vaikutus molemmilla sukupuolilla eli keskiarvo näyttäisi kasvattavan tulotasoja maltillisen lineaarisesti. Toisaalta mallin sovittamisvaiheessa voitaisiin myös testata mallia jossa otetaan selittäjäksi koulukeskiarvon ja sukupuolen interaktio.

```{r}
ggplot(data=d, mapping=aes(x=ITPA, y=tulot, colour = sukupuoli))+
  geom_point()

#ITPA-älykkyystestin tulos näyttää kasvattavan tuloja lineaarisesti. Naisten ja miesten välillä
#ei ole suuria eroja.

ggplot(data=d, mapping=aes(x=koulutustaso, y=tulot))+
  geom_boxplot()

#Koulutusasteella näyttää olevan olennainen vaikutus tuloihin vain ylemmän korkeakouluasteen ja muiden välillä. Voisimme siis pelkistää dataa.

d$ylempikk <- ifelse(d$koulutustaso == "Ylempi korkeakouluaste", "Kyllä", "Ei")
```

# Mallinnus (mieti otsikko)

Priorien valinta: Priorin valinnassa voisi huomioida ennakkokäsitysken esim. sukupuolen vaikutuksesta palkkaan tai kouluarvosanojen keskiarvon merkityksen.

Sovitetaan ensin malli jossa selittäjinä ovat arvosana, ITPA, sukupuoli ja ylempikk.

```{r}

# fit1 <- brm(tulot ~ arvosana + ITPA + sukupuoli + ylempikk, data=d)
# fit1
# 
# fit2_def <- brm(tulot ~ arvosana + sukupuoli + ylempikk, data=d)
# fit2_def
# 
# #Esimerkkiprioreja
# priors <- c(
#   prior(normal(-5000, 5000), class="b", coef="sukupuoliNainen"),
#   prior(normal(0,5000), class="b", coef="arvosana"),
#   prior(normal(10000, 5000), class="b", coef="ylempikkKyllä"),
#   prior(student_t(3, 18874.5, 9625.4), class="Intercept"),
#   prior(student_t(3, 0, 9625.4), class="sigma")
#   )
# 
# #prioriennustejakaumaa varten
# fit2_prior <- brm(tulot ~ arvosana + sukupuoli + ylempikk, prior = priors, sample_prior = "only", data=d)
# 
# pp_check(fit2_prior)
# 
# fit2_set <- brm(tulot ~ arvosana + sukupuoli + ylempikk, prior = priors, data=d)
# fit2_set
# 
# #Mallien vertailu LOO-avulla
# loo1 <- loo(fit2_def)
# loo2 <- loo(fit2_set)
# 
# loo_compare(loo1, loo2)
# 
# plot(conditional_effects(fit2_set), points = TRUE)
# 
# get_prior(tulot ~ arvosana + ITPA + sukupuoli + ylempikk, data=d)

```

# Lähdeviitteet
