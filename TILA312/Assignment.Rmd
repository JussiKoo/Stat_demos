...
assuming that the first $k$ observations are $0$. So the model parameters can be solved from separate estimating equations.

\begin{equation}
\begin{split}
l'(\gamma) &= \frac{\partial}{\partial \gamma} \left(\sum_{i=1}^k \log f_{zero}(0;z_i,\gamma) + \sum_{i=k+1}^n \log(1 - f_{zero}(0;z_i,\gamma))\right) = 0\\
\end{split}
\end{equation}
...
summary(fitall)
```
Significance of gender and race is in line with what we saw earlier so they are dropped from the model. Flame is also dropped since it seems like it is not significant. Let's see if interaction with inhalation injury is needed.
...
#Small p-value from Hosmer-Lemeshow test would indicate bad fit.
#p-value is quite high now so we can't reject the model.
hoslem.test(data$death, fitted(fit1))
 
#Pearson residuals.
r <- residuals(fit1, type = "pearson")

sum(resid(fit1, type = "pearson")^3) / df.residual(fit1)

```
Let's use the model to make predictions for test data. If probability to die is over 0,5 let's say that the prediction is that patient will die.
...
The final model could predict the outcome in the test data with 91 % accuracy. This seems like a reasonably good result.

The models assumptions are that all observations are independent and relationships between predictors and log-odds are linear. Model can be interpreted using odds and odds ratios (OR) and in probability level.
...
b[1]
```
Let's examine the effect of total burnt surface area with odds and odds ratios.
