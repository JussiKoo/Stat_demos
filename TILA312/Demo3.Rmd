...
fit1 <- gls(sciescore ~ matem + motiv + SCHOOLID + gender + ESCS + urban + region,
...
#reduced model
fit2 <- gls(sciescore ~ matem + SCHOOLID + gender + ESCS + motiv + urban,
            correlation = corCompSymm(form = ~ 1 | SCHOOLID),
            method="ML")
...
similar F-test for other variables we say that matem, SCHOOLID, ESCS and motiv are
significant. Interaction between matem and SCHOOLID is needed and interaction 
between matem and motiv is needed. I used anova to investigate significance of
interactions.

```{r}
#g

fit <- gls(sciescore ~ matem * SCHOOLID + matem * motiv + ESCS,
            correlation = corCompSymm(form = ~ 1 | SCHOOLID),
            method="ML")
summary(fit)
...
fitreg <- gls(sciescore ~ matem * SCHOOLID + matem * motiv + ESCS,
            method="ML")

anova(fit, fitreg)

#general model has slightly lower AIC so it's better.

#j

fitREML <- gls(sciescore ~ matem * SCHOOLID + matem * motiv + ESCS,
            correlation = corCompSymm(form = ~ 1 | SCHOOLID),
            method="REML")
summary(fitREML)
